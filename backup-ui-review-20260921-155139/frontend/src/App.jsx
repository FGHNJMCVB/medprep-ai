...
function App() {
  ...
  const SUBJECT_MOCK_QUESTION_COUNTS = [
    50,
    100,
    150
  ];
  ...
  const [subjectMockOpen, setSubjectMockOpen] =
    useState(false);

  const [subjectMockSubjectId, setSubjectMockSubjectId] =
    useState("");

  const [subjectMockQuestionCount, setSubjectMockQuestionCount] =
    useState(150);
      ...
      try {
        ...
        if (!saved?.session) {
          ...
          const partDurationSeconds =
            isSubjectMockSession(serverSession)
              ? (Number(serverSession.totalQuestions) ||
                  serverSession.questions.length) * 60
  ...
  function handleLogout() {
    ...
    setSubjectMockSubjectId("");
    setSubjectMockQuestionCount(150);
  ...
  async function handleStartSubjectMock() {
    const subjectId = Number(subjectMockSubjectId);
    const questionCount = Number(subjectMockQuestionCount);

    if (!subjectId) {
      setError("Choose a subject for the mock test.");
      return;
    }
    ...
    try {
      const mock = await startSubjectMockTest(
        subjectId,
        questionCount
      );
      ...
      if (
        !mock ||
        !Array.isArray(mock.questions) ||
        mock.questions.length !== questionCount
      ) {
        throw new Error(
          `Subject mock test must contain exactly ${questionCount} questions, but ${mock?.questions?.length ?? 0} were returned.`
        );
      }
      ...
      console.error(
        "Failed to start subject mock test:",
        startError
      );

      setError(
        startError?.message ||
          "Unable to start subject mock test."
      );
    } finally {
      setLoading(false);
    }
  ...
  function isSubjectMockSession(
    candidateSession = session
  ) {
    return candidateSession?.type ===
      "SUBJECT_MOCK_TEST";
  }

  function getMockPartQuestionCount() {
    if (isSubjectMockSession()) {
    ...
    const confirmed =
      window.confirm(
        unanswered > 0
          ...
          : `Submit your ${isSubjectMockSession() ? "subject" : "FMGE"} mock test? You will not be able to change your answers after submission.`
      );
    ...
    return (
      <div className="app-shell">
        ...
        <main className="dashboard">
          ...
          <section className="hero">
            <div className="hero-content">
              ...
              <div className="hero-actions">
                ...
                <button
                  type="button"
                  className="hero-secondary-button large"
                  onClick={() => {
                    setSubjectMockOpen(previous => !previous);

                    if (
                      !subjectMockSubjectId &&
                      subjects[0]?.id
                    ) {
                      setSubjectMockSubjectId(
          ...
          {subjectMockOpen && (
            <section className="subject-mock-panel">
              <div className="subject-mock-copy">
                <p className="eyebrow">TIMED SUBJECT EXAM</p>

                <h2>Build a subject mock</h2>

                <p>
                  Questions are selected from all available topics in the
                  subject, with a balanced mix of easy, medium, and hard
                  questions. Each question gets one minute.
                </p>
              </div>

              <div className="subject-mock-form">
                <label htmlFor="subject-mock-subject">
                  Subject
                </label>

                <select
                  id="subject-mock-subject"
                  value={subjectMockSubjectId}
                  onChange={event =>
                    setSubjectMockSubjectId(event.target.value)
                  }
                  disabled={loading}
                >
                ...
                <span className="subject-mock-label">
                  Number of questions
                </span>

                <div className="subject-mock-counts">
                  {SUBJECT_MOCK_QUESTION_COUNTS.map(count => (
                    <button
                      key={count}
                      type="button"
                      className={
                        subjectMockQuestionCount === count
                          ? "subject-mock-count selected"
                          : "subject-mock-count"
                      }
                      onClick={() =>
                        setSubjectMockQuestionCount(count)
                      }
                      disabled={loading}
                    >
                      {count}
                ...
                <div className="subject-mock-summary">
                  <span>{subjectMockQuestionCount} questions</span>
                  <span>{subjectMockQuestionCount} minutes</span>
                  <span>1 part</span>
                </div>

                <button
                  type="button"
                  className="primary-button large subject-mock-start"
                  onClick={handleStartSubjectMock}
                  disabled={loading || !subjectMockSubjectId}
                >
                  {loading
                    ? "Preparing..."
                    : "Start Subject Mock"}
  ...
  if (screen === "exam") {
    ...
    const mockSubjectName =
      subjects.find(subject =>
        Number(subject.id) === Number(session.subjectId)
      )?.name || "Subject";

    const mockTitle = subjectMock
      ? `${mockSubjectName} Mock Test`
      : "FMGE Mock Test";
    ...
    if (showPartTransition && !subjectMock) {
      return (
        <div className="exam-shell">
          <header className="exam-header">
            <div>
            ...
            <div className="question-card">
              ...
              >
                ...
                >
                  ...
                  >
                    {!subjectMock && mockPart === 1 && currentIndex === partEndIndex
                      ? "Part 1 Complete →"
                      : "Next →"}
                  </button>
                </div>
